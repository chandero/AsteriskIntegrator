...
public class RecordingServlet
    ...
    private static final long serialVersionUID = -7449507290127731703L;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //try {
    		...
    		String requestedFile = file.getAbsolutePath();
    		finalfile = tmpFolder+file.getName();
    		finalfile = finalfile.replace(".WAV", ".wav");
    		finalfile = finalfile.replace(".gsm", ".wav");
    		System.out.println("Final File Name="+finalfile);    		
    		//String basePath = file.getAbsolutePath();
    		File audioConvertido = null;
    		if (requestedFile.endsWith(".WAV")) {
    				CommonUtils.executeCommand("sox " + requestedFile + " -s " + finalfile);
    		} if (requestedFile.endsWith(".gsm")) {
    			CommonUtils.executeCommand("sox " +  requestedFile + " -t wav " + finalfile);
    		}
			CommonUtils.executeCommand("ffmpeg -i " + finalfile + " -codec:a libmp3lame -qscale:a 2 " + finalfile.replace(".wav", ".mp3"));
			String convertido = finalfile.replace(".wav", ".mp3");
    		audioConvertido = new File(convertido);
        
    		returnRequest(audioConvertido, request, response, true);                
/*                String mime = context.getMimeType(recordFile.getAbsolutePath());
    ...
    */
        ...
        (File file, HttpServletRequest request, HttpServletResponse response, boolean content)
            throws IOException
    {
       ...
       // File file = new File(basePath, URLDecoder.decode(requestedFile, "UTF-8"));
        ...
        // Initialize response.
        String fileNameToDownload = fileName.split("\\.")[0]+".wav";
        response.reset();
        response.setBufferSize(DEFAULT_BUFFER_SIZE);
        response.setHeader("Content-Disposition", disposition + ";filename=\"" + fileNameToDownload + "\"");
